...
cinderKeyring=`cat /etc/ceph/ceph.client.cinder.keyring` || exit 1
keyring=`echo "${cinderKeyring}" | grep 'key = ' | awk -F\ \=\  '{print $2}'` || exit 1
[ "${keyring}" ] || exit 1
...
kubectl create -f ${programDir}/../deployment/flexvolume/flexvolume-ds.yaml  ||  exit 1
